 ...
 $id = $_POST['idsubject'];
   ...
   ');
    ...
    <form action="" method=""  id="smsform">
        ...
        <textarea name="body" id="" cols="30" rows="10" style="height: 50px;resize:none;padding:10px" >hello '.$fmemberfirstname.' we will like to inform you that '.$firstname.' '.$lastname.' did not come to school hope all is well @'.$school.'</textarea>
   ...
   ');
    ...
    $date = date('Y-m-d');
    $query = "SELECT * FROM students WHERE school ='$school' AND campus = '$campusselect' AND lastattendance NOT LIKE '$date%' AND class = '$class'";
    $send = $conn->query($query);
     $resultnum = mysqli_num_rows($send);
    $i = 0;
    ...
    <input type="hidden" id="'.$uniqe.'" value="'.$campusselect.'" class="'.$class.'" >
     ...
     <td >'.$i.'</td>
     <td><span class="studentt text-primary text-decoration-underline" onclick="a('.$uniqe.')" style="cursor: pointer;">'.$uniqe.'</span></td>
    ...
    if(isset($teachername['studentcampus3']) AND isset($teachername['uniqe3']) AND isset($teachername['class'])){

        $campusselect = $teachername['studentcampus3']; 
        $teacherid = $teachername['uniqe3'];
        $class = $teachername['class'];
       $query = "INSERT INTO studentatt (school,campus,uniqe,`datetime`,class,`status`) VALUES ('$school','$campusselect','$teacherid',CURRENT_TIMESTAMP(),'$class','present')";
       $send = $conn->query($query);
       $query = "UPDATE students SET lastattendance = CURRENT_TIMESTAMP()";
       $send = $conn->query($query);
    } 
    if(isset($teachername['studentcampus4']) AND isset($teachername['uniqe4']) AND isset($teachername['class'])){

        $campusselect = $teachername['studentcampus4']; 
        $teacherid = $teachername['uniqe4'];
        $class = $teachername['class'];
       $query = "INSERT INTO studentatt (school,campus,uniqe,`datetime`,class,`status`) VALUES ('$school','$campusselect','$teacherid',CURRENT_TIMESTAMP(),'$class','absent')";
       $send = $conn->query($query);
    }  $query = "UPDATE students SET lastattendance = CURRENT_TIMESTAMP()";
    $send = $conn->query($query);
}
...
<div class="  d-none popperclasslist  popperdetails position-fixed d-flex justify-content-center align-items-center " style="height: 100%; width:100% ;z-index: 10; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.808);">
    ...
    <form action="" class="col-12 d-none" >
        ...
        <div class="h-100" style="overflow:scroll;max-width:100%">

          
        <table class="tablem table-striped  thestudentlistsz " style="text-transform: capitalize;"  >
...
<div class=" d-none popperteachersdetails popperdetails position-fixed d-flex justify-content-center align-items-center " style="height: 100%; width:100% ;z-index: 10; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.808);">
    ...
    <div class="col-12 col-md-6 bg-light position-relative" style="border-radius: 10px;height: 90%; padding: 10px;">
    <button class="btn btn-danger float-end close">X</button>
    <div class="col-12 pt-5  d-flex flex-column g-2 align-items-center appendstudentdetailse" style="height: 90%; overflow-y: scroll; padding-bottom: 1cm ;">
...
<!-- -->
               ...
               <form action="" class="col-12 d-none <?=$none?>" >
                <div class="input-group mt-2 form-control w-100">
                  ...
                  $resultnum2 = mysqli_num_rows($send2);
                    ...
                    for($i=0;$i<$resultnum2;$i++){
                     ...
                     $j;
                      ...
                      $querystudent = "SELECT * FROM students WHERE school ='$school' AND campus = '$campusselect' AND lastattendance NOT LIKE '$d%'AND class='$class' ";
    ...
    <script>
        ...
        xml.onreadystatechange =()=>{
            if(xml.readyState == 4 && xml.status == 200){
                ...
                let studentdetails = document.querySelector('.appendstudentdetailse');
        ...
        function b(a){
            ...
            data = JSON.stringify(data);
            let xml = new XMLHttpRequest;
        xml.open('POST','attendance-students.php');
        xml.onreadystatechange =()=>{
            if(xml.readyState == 4 && xml.status == 200){
                let response = xml.responseText;
