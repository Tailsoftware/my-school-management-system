...
if(isset($_POST['deleteprefect'])){
    $id = $_POST['deleteprefect'];
    $query ="UPDATE students SET position ='' WHERE uniqe ='$id' AND school ='$school' AND campus ='$campusselect'";
    $send = $conn->query($query);
}
  ?>
...
<div class=" d-none popperteachersdetails popperdetails position-fixed d-flex justify-content-center align-items-center " style="height: 100%; width:100% ;z-index: 10; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.808);">
    ...
    <div class="col-12 pt-5  d-flex flex-column g-2 align-items-center  appentteachersresponse" style="height: 90%; overflow-y: scroll; padding-bottom: 1cm ;">
     ...
     <div class="col-12 col-md-7 h-75   position-relative loopt"  style="padding: 10px;max-width: 100vw;" >
               <a href="registrer-student.php" class="btn btn-dark text-capitalize"> Add new prefect</a>
   ...
   <?php
     $query2 = "SELECT * FROM students WHERE school = '$school' AND campus = '$campusselect' AND position != ''";
  ...
  $resultnum2 = mysqli_num_rows($send2);
     ...
     for($i=0;$i<$resultnum2;$i++){
      ...
      $j++;
        ...
        $lastname = $result2['lastname'];
         ...
         $query = "SELECT * FROM teachers WHERE school = '$school' AND campus = '$campusselect' AND class = '$class'";
         $send= $conn->query($query);
         $resultnum = mysqli_num_rows($send);
         if($resultnum>0){
            $result = mysqli_fetch_assoc($send);
            $teacherf = $result['firstname'];
            $teacherl = $result['lastname'];
    ...
    ?>
                ...
                <tr class="d" style="">
                    <td><?=$j?></td>
                    <td style=" line-break: break-all;"> <a href="" class="teachersdetails2"><?=$id?> <input type="hidden" name="teacherid" value="<?=$id?>" class="teacherid"> <input type="hidden" class="campusselect" value="<?=$campusselect?>"></a></td>
                    <td><?=$gender?></td>
                    <td><?=$firstname?></td>
                    <td><?=$lastname?></td>
                    <td><?=$class?></td>
                    <td style="word-wrap: break-word;"><?=$teacher?></td>
                    <td class="dc"><?=$position?></td>
                    <td>
                    <form action="<?=$_SERVER['PHP_SELF']?>" method="POST"> <input type="hidden" value="<?=$id?>" name="deleteprefect"> <button class="btn btn-danger">Delete</button></form>
