...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
   ...
   ');
        ...
        $query = "SELECT * FROM admin WHERE school = '$school' AND email ='$teacherid'";
    $send = $conn->query($query);
    $result = mysqli_fetch_assoc($send);
    $resultnum = mysqli_num_rows($send);
    if($resultnum >0){
    ...
    $qualification = 'ADMIN';
    $fmember = 'ADMIN';
    $fmemberfirstname = 'ADMIN';
    $fmemberlastname = 'ADMIN';
    $fnumber = 'ADMIN';
    $email = 'ADMIN';
    $profileimg = $result['profileimg'];
    exit('  
...
<!-- link -->
        ...
        <!-- body -->
        <div class="body flex-grow-1 bg-light  align-items-center d-flex " style="height: 100%;overflow:scroll;">
 ...
 <div class="  d-none popperstudent popperdetails position-fixed d-flex justify-content-center align-items-center " style="height: 100%; width:100% ;z-index: 101; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.808);">
  ...
  <div class=" d-none popperteachersdetails popperdetails position-fixed d-flex justify-content-center align-items-center " style="height: 100%; width:100% ;z-index: 10; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.808);">
    <div class="col-12 col-md-6 bg-light position-relative" style="border-radius: 10px;height: 90%; padding: 10px;">
    <button class="btn btn-danger float-end close">X</button>
    <div class="col-12 pt-5  d-flex flex-column g-2 align-items-center  appentteachersresponsel" style="height: 90%; overflow-y: scroll; padding-bottom: 1cm ;">
...
<!-- e -->
   ...
   <?php
                ...
                $query2 = "SELECT * FROM loanedbook WHERE school ='$school' AND campus = '$campusselect' ORDER BY id DESC";
   ...
   $resultnum2 = mysqli_num_rows($send2);
     ...
     while( $result2 = mysqli_fetch_assoc($send2)){
      ...
      $j++;
         ...
         $loanto = $result2['loanto'];
         $loanby = $result2['loanby'];
         $date = $result2['date'];
         $book = $result2['book'];
...
<!-- s -->
                        <tr class="d">
                        <td class="dc"><?=$j?></td>
                        <input type="hidden" id="<?=$loanto?>" value="<?=$campusselect?>" class="<?=$class?>" >

                        <td><span class="student text-primary text-decoration-underline" onclick="a('<?=$loanto?>','<?=$campusselect?>')" style="cursor: pointer;"><?=$loanto?></span></td>
                        <td class="dc" style=" line-break: break-all;"> <a href="" class="teachersdetails"><?=$loanby?> <input type="hidden" name="teacherid" value="<?=$loanby?>" class="teacherid"> <input type="hidden" class="campusselect" value="<?=$campusselect?>"></a></td>
                        <td class="dc"><?=$book?></td>
                        <td class="dc"><?=$number?></td>
                        <td class="dc"><?=$date?></td>
