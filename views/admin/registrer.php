...
if($section == 'admin'){
  ...
  if(isset($_FILES['profile'])){
    if($_FILES['profile']['size']>0){
      ...
      $tmpfolder = $_FILES['profile']['tmp_name'];
      ...
      if(in_array($ext,$extarray)){
        if(file_exists($destination)){
          ...
          $query = "UPDATE `admin` SET profileimg = '$imgname' WHERE uniqe = '$uniqe'";
          $send = $conn->query($query);
    ...
    if($_FILES['profile']['size']>0){
      ...
      if(in_array($ext,$extarray)){
        if(file_exists($destination)){
          ...
          $destination = '../upload/'.$imgname;
          move_uploaded_file($tmpfolder,$destination);
          $query = "INSERT INTO teachers SET profileimg = '$imgname' WHERE uniqe = '$uniqe'";
    ...
    if($_FILES['profile']['size']>0){
      ...
      if(in_array($ext,$extarray)){
        if(file_exists($destination)){
          ...
          $destination = '../upload/'.$imgname;
          move_uploaded_file($tmpfolder,$destination);
          $query = "INSERT INTO `officestaf` SET profileimg = '$imgname' WHERE uniqe = '$uniqe'";
          $send = mysqli_query($conn,$query);
        }
      }
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
   ...
   if(isset($_POST['firstname']) AND isset($_POST['lastname'])  AND isset($_POST['salary'])  AND isset($_POST['class'])  AND isset($_POST['homeaddr'])  AND isset($_POST['gender'])  AND isset($_POST['qualification'])  AND isset($_POST['fmember'])  AND isset($_POST['ffirstname'])  AND isset($_POST['flastname'])  AND isset($_POST['email'])  AND isset($_POST['campus']) AND isset($_POST['phonenumber']) AND isset($_POST['fnumber'])  ){
   
   
      if($_POST['firstname'] !="" AND  $_POST['lastname']!="" AND  $_POST['salary']!="" AND $_POST['class'] !=""  AND $_POST['homeaddr'] !="" AND $_POST['gender'] !="" AND $_POST['qualification'] !="" AND $_POST['fmember'] !="" AND $_POST['ffirstname'] !="" AND $_POST['flastname'] !="" AND $_POST['email'] !="" AND $_POST['phonenumber'] !="" AND $_POST['campus'] !="" AND $_POST['fnumber'] !=""){
      ...
      $salary = trim( htmlspecialchars(filter_input(INPUT_POST,'salary',FILTER_SANITIZE_SPECIAL_CHARS)));
      $gender = trim( htmlspecialchars(filter_input(INPUT_POST,'gender',FILTER_SANITIZE_SPECIAL_CHARS)));
      $qualification = trim( htmlspecialchars(filter_input(INPUT_POST,'qualification',FILTER_SANITIZE_SPECIAL_CHARS)));
     ...
     $name = $firstname.' '.$lastname;
      ...
      $resultnum = mysqli_num_rows($send);
      if($resultnum > 0){
         echo"<script>alert('user with email exist')</script>";
         header("location:registrer.php");
      }
      else{
         $query ="INSERT INTO `teachers`(`sec`, `firstname`, `lastname`, `uniqe`, `class`, `section`, `homeaddr`, `gender`,`qualification`, `fmember`, `fmemberfirstname`, `fmemberlastname`, `email`, `phonenumber`,salary,fnumber,`password`,`campus`,school,name,lastattendance) VALUES ('$sec','$firstname','$lastname','$uniq','$class','$section','$homeaddr','$gender','$qualification','$fmember','$ffirstname','$flastname','$email','$phonenumber','$salary','$fnumber','$password','$campus','$school','$name',CURRENT_TIMESTAMP())";
...
if(isset($_POST['campus']) AND empty($_POST['lastname'])){
   if($_POST['campus'] !="" ){
      $campus = trim(htmlspecialchars($_POST['campus']));

      $query = "SELECT * FROM campus WHERE campus = '$campus'";
      $send = $conn->query($query);
      $resultnum = mysqli_num_rows($send);
      if($resultnum > 0){
         header("location:registrer.php?message='campus already exist'");
      }
      else{

         $query = "INSERT INTO campus (school,campus,email) VALUES('$school','$campus','$email')";
         $send = $conn->query($query);
...
if(isset($_POST['campdel'])){
   if($_POST['campdel'] !=''){
      $campus = $_POST['campdel'];
      $query = "DELETE FROM campus WHERE campus = '$campus'";
      $send = $conn->query($query);
...
?>
     ...
     <div class="body flex-grow-1 bg-light  align-items-center d-flex " style="height: 100%;"> 
        <div class="h" style=" "> <button class="btn btn-dark addcampus" >Add campus</button> <a href="registrer.php" class="btn  btn-primary" style="text-align: center;">teacher</a>    <a href="registrer-student.php" class="btn btn-secondary btn-dark" >student</a>   <a href="officestaffreg.php" class="btn btn-dark">office staff</a></div>
