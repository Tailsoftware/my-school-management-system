...
if(isset($_POST['attuniqe'])){
   ...
   $time = time();
    ...
    $query ="SELECT * FROM officestaf WHERE uniqe ='$uniq'";
    $send = $conn->query($query);
   $resultnum = mysqli_num_rows($send);
    if($resultnum>0){
        $date = date('Y-m-d');
        ...
        if($resultnum>0){
            ...
            $query ="INSERT INTO attendance (uniqe,section,school,campus,time) VALUES ('$uniq','staff','$school','$campusselect','$time')";
            $send = $conn->query($query);
            $query ="UPDATE officestaf SET lastattendance ='$date' WHERE uniqe ='$uniq'";
            $send = $conn->query($query);
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $query = "SELECT * FROM officestaf WHERE school = '$school' AND campus ='$campusselect' AND uniqe ='$teacherid'";
    $send = $conn->query($query);
    $result = mysqli_fetch_assoc($send);
    $resultnum = mysqli_num_rows($send);
...
<div class="d-none  popperregular popperdetails position-fixed d-flex justify-content-center align-items-center " style="height: 100%; width:100% ;z-index: 10; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.808);">
    ...
    <div class="col-12 pt-5  d-flex flex-column g-2 align-items-center" style="height: 90%; overflow-y: scroll; padding-bottom: 1cm ;">
        ...
        <div class="table-responsive overflow-scroll mt-5 " >

        <table class="table table-striped table-bordered appentteachersresponser2" style="text-transform: capitalize;">
                 <!-- s -->
                 <tr class="table-active" style="font-weight: 600;">
                         <td>#</td>
                         <td>gender</td>
  ...
  <div class=" d-none popperteachersdetails popperdetails position-fixed d-flex justify-content-center align-items-center " style="height: 100%; width:100% ;z-index: 10; top: 0; left: 0; background-color: rgba(0, 0, 0, 0.808);">
    ...
    <div class="col-12 pt-5  d-flex flex-column g-2 align-items-center  appentteachersresponse" style="height: 90%; overflow-y: scroll; padding-bottom: 1cm ;">
     ...
     <div class="col-12 col-md-8 h-75  m-auto position-relative box2" style="padding: 10px;">
            <a href="attendance-teacher.php" class="btn btn-dark text-capitalize">teachers</a>
            <a href="attendance-staff.php" class="btn btn-primary text-capitalize">staff</a>
            <a href="attendance-students.php" class="btn btn-dark text-capitalize <?=$attstu?>">students</a>
              ...
              <div class="input-group mt-3" style="width: cm;">
               ...
               <table class=" tablem d-flex flex-column" style="text-transform: capitalize;" id="classtable" >
                ...
                <tr class="table-active" style="font-weight: 600;">
                 ...
                 $date=date('Y-m-d');
                 $query = "SELECT * FROM attendance WHERE section = 'staff'AND school='$school' AND campus='$campusselect' AND `datetime` LIKE '$date%'";
                 $send = $conn->query($query);
                 $resultnum = mysqli_num_rows($send);
                 $j=0;
                 while($result = mysqli_fetch_assoc($send)){
                  ...
                  $query2 = "SELECT * FROM officestaf WHERE uniqe = '$uniqe' AND school='$school' AND campus='$campusselect'";
                    ...
                    if($resultnum2>0){
                     ...
                     <!-- s -->
                         <tr class="" style="">
                            ...
                            <td><?=$time?></td>
                            <td>
                            <a href="" class="teachersdetails3 btn btn-warning">view <input type="hidden" name="teacherid" value="<?=$id?>" class="teacherid"> <input type="hidden" class="campusselect" value="<?=$campusselect?>"></a>                         <a href="" class="btn btn-danger">Delete</a>
